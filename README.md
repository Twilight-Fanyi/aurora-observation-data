...
`/v1/weather.json` 是生成端使用的内部缓存，不属于鸿蒙客户端 API 合约。缓存会校验 50 个地点的数量、顺序、时间和数值范围；Open-Meteo 刷新失败时可复用不超过 3 小时的天气缓存，NOAA 27 天趋势每 24 小时刷新一次并可短期降级复用。超过安全缓存期限时保留上一份完整发布，避免混合新旧数据。

## 数据内容

- NOAA SWPC：当前与预报 Kp、Bz、太阳风速度、OVATION 极光椭圆；短期预报汇总为 3 个地点当地日期，另提供一份全局共享的 27 天太阳活动趋势（F10.7、Ap 与预测最大 Kp）。
- Open-Meteo：各地点 16 天逐小时云量与能见度，汇总为每日最佳夜空条件。
- 本仓库：保留未来 12 小时整点预报，并生成 3 天极光机会摘要、16 天夜间天气趋势与 27 天太阳活动趋势。

“观测机会指数”是帮助安排观测的派生指标，不是极光出现概率，也不构成安全或出行保证。16 天数据只描述地面夜间天气；27 天数据只反映全球太阳与地磁活动趋势，不等同于当地极光可见概率。中国低纬地点通常需要较强地磁活动才有机会看到极光。
...
流水线不接收或保存用户位置、设备标识、账号与个人资料，不需要 API Key。按 50 个地点、15 分钟刷新估算，Open-Meteo 计量约为每天 4,800 次、31 天月份 148,800 次，低于非商业免费层每天 10,000 次与每月 300,000 次的公开额度。NOAA 27 天趋势全局只抓取一份、每天最多刷新一次，不随地点数量增长。Cloudflare 只镜像并校验客户端所需 JSON，不维护第二套评分逻辑。
